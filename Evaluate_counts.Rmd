...
```{python}
def plot_counts(folder_inference, tile, folder_annotations, group='group1', preds_axis=0):
    ...
    pred_count = preds[~mask_out].sum()

    diff = raster  - preds1
    diff1 = (raster - preds1 )/ raster

    fig = plt.figure(figsize=(15,5))

    plt.subplot(1, 3, 1)

    im = plot_heatmap(data=raster,min=0,max=2)
    plt.imshow(im)
    # plt.imshow(raster)
    plt.title(f'GT {gt_count:.2f}')

    plt.subplot(1, 3, 2)
    # plt.imshow(preds)
    im = plot_heatmap(data=preds,min=0,max=2)
    plt.imshow(im)

    plt.title(f'pred {pred_count:.2f}')
    plt.suptitle(f' {np.nansum(1.1*(preds1>0.1)):.2f}')

    plt.subplot(1, 3, 3)
    im = plot_heatmap(data=diff,min=-2,max=2,cmap='seismic')
    plt.imshow(im)
    # plt.imshow(preds1 -  raster,cmap='seismic')

    plt.title(f'diff {np.nansum(diff):.2f},   {-np.nansum(diff)/np.nansum(raster):.2f}% ')

    plt.suptitle(f' {tile} {group}')

```

```{python}
plot_counts(folder_inference='/scratch/andresro/leon_work/sparse/inference/palmcoco_kalimA_simpleA5',
            tile='T49MCV',
            folder_annotations='/scratch/andresro/leon_igp/barry_palm/data/labels/coco_annotations',
            group='group1',preds_axis=1)
plot_counts(folder_inference='/scratch/andresro/leon_work/sparse/inference/palmcoco_kalimA_simpleA5',
            tile='T49MCV',
            folder_annotations='/scratch/andresro/leon_igp/barry_palm/data/labels/coco_annotations',
            group='group2',preds_axis=1)
plot_counts(folder_inference='/scratch/andresro/leon_work/sparse/inference/palmcoco_kalimA_simpleA5',
            tile='T49MCV',
            folder_annotations='/scratch/andresro/leon_igp/barry_palm/data/labels/palm_annotations',
            group='group1',preds_axis=0)
plot_counts(folder_inference='/scratch/andresro/leon_work/sparse/inference/palmcoco_kalimA_simpleA5',
            tile='T49MCV',
            folder_annotations='/scratch/andresro/leon_igp/barry_palm/data/labels/palm_annotations',
            group='group2',preds_axis=0)
```

```{python}
plot_counts(folder_inference='/scratch/andresro/leon_work/sparse/inference/palmsarawak_simpleA9',
            tile='T49NFC',
            folder_annotations='/scratch/andresro/leon_igp/barry_palm/data/labels/palm_annotations',
            group='group2')
plot_counts(folder_inference='/scratch/andresro/leon_work/sparse/inference/palmsarawak_simpleA9',
...
```{python}
preds1[preds1 > 0.1].mean(), raster[raster > 0.1].mean()
