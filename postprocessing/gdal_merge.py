...
# =============================================================================
def get_dist_to_border(arr, max_dist=None, normed=False):
    h, w = arr.shape[0:2]

    # dist from left
    dist_l = np.matlib.repmat(np.arange(w), h, 1)
    # dist from right
    dist_t = np.transpose(np.matlib.repmat(np.arange(h), w, 1))
    # dist from right
    dist_r = np.fliplr(dist_l)
    # dist from bottom
    dist_b = np.flipud(dist_t)

    dist_tl = np.minimum(dist_t, dist_l)
    dist_br = np.minimum(dist_b, dist_r)

    dist_matrix = np.minimum(dist_tl, dist_br)
    ...
    if normed:
        dist_matrix = np.divide(dist_matrix, float(max_dist))
...
def raster_copy( s_fh, s_xoff, s_yoff, s_xsize, s_ysize, s_band_n,
    ...
    if crossfade:
        return raster_copy_with_crossfade(
            s_fh, s_xoff, s_yoff, s_xsize, s_ysize, s_band_n,
            t_fh, t_xoff, t_yoff, t_xsize, t_ysize, t_band_n,
            overlap=900)
...
# =============================================================================
def get_crossfade_weights(data_src, data_dst, overlap):
    weights_src = get_dist_to_border(arr=data_src, max_dist=overlap, normed=True)

    # weight only overlapping pixels (data_dst is not empty)
    weights_src[np.isnan(data_dst)] = 1.0
    weights_src[np.isnan(data_src)] = 0
    weights_dst = 1.0 - weights_src

    return weights_src, weights_dst


# if target array is already filled, then crossfade the new array at the borders
def raster_copy_with_crossfade( s_fh, s_xoff, s_yoff, s_xsize, s_ysize, s_band_n,
                           t_fh, t_xoff, t_yoff, t_xsize, t_ysize, t_band_n,
                           overlap=32):
    ...
    # weighted average between src and dst
    weights_src, weights_dst = get_crossfade_weights(data_src=data_src, data_dst=data_dst, overlap=overlap)

    # update based on new weights
    data_dst = np.multiply(data_dst, weights_dst)
    data_src = np.multiply(data_src, weights_src)

    # pixels empty in source and destination
    empty_mask = np.logical_and(np.isnan(data_src), np.isnan(data_dst))

    # add to valid pixels, replace nans with zeros
    data_dst[np.isnan(data_dst)] = 0
    data_src[np.isnan(data_src)] = 0
    to_write = data_src + data_dst

    # set nan values again
    to_write[empty_mask] = np.nan
...
class file_info:
    ...
    def copy_into( self, t_fh, s_band = 1, t_band = 1, nodata_arg=None , crossfade=False):
        """
        ...
        return raster_copy( s_fh, sw_xoff, sw_yoff, sw_xsize, sw_ysize, s_band,
                            ...
                            nodata_arg, crossfade=crossfade)
...
def main(names,out_file = 'out.tif', args=None ):
    ...
    for fi in file_infos:
        ...
        if separate == 0 :
            for band in range(1, bands+1):
                fi.copy_into( t_fh, band, band, nodata, crossfade)
        else:
